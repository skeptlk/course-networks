...
class MyTCPProtocol(UDPBasedProtocol):
    ...
    def working_thread(self):
        while not self.halt:
            try:
                ...
                if packet.id not in self.used_ids:
                    if packet.id - self.ack != 1:
                        ...
                        if packet.is_empty():
                            self.log("empty ack =", packet.ack, " split=", packet.split)
                            if packet.split == 5:
                                self.send_lost_packets(packet.ack)
                            else:
                                self.remove_acked_packets(packet.ack)
                        else:
            ...
            # if self.time - self.last_sent_time >= 50 * self.TICK and len(self.sent_packets) > 0:
            ...
            if self.time - self.last_sent_time >= 150 * self.TICK:
                if len(self.send_queue) == 0:
    ...
    def send_lost_packets(self, ack: int):
        ...
        for p in lost_packets:
            p.ack = self.ack
        self.send_queue = self.send_queue + lost_packets
        self.send_queue = list(OrderedDict.fromkeys(self.send_queue))

        self.log("Queue after: ", [p.id for p in self.send_queue])

    def remove_acked_packets(self, ack: int):
        self.send_queue = list(filter(lambda x: x.id > ack and not x.is_empty(), self.send_queue))
